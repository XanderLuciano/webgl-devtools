define(["messages", "jsx!profile_table", "jsx!profile_graph"],
    function (Messages, ProfileTable, ProfileGraph) {
    return React.createClass({
        ...
        getTabNames: function() {
            const tabNames            = ["Profiles"];
            let programUsage          = 0;
            let duplicateProgramUsage = 0;
            let callStack             = 0;
            let drawCallStack         = 0;
            let histogram             = 0;
        ...
        getTabs: function() {
            ...
            for (let i = 0; i < tabNames.length; i++) {
                let className = "split-view-table-element";
                if (this.state.selectedTab === i) {
                    className += " split-view-table-element-selected";
                }

                const el = <div className={className} onClick={this.switchTab.bind(this, i)}>
        ...
        onProfileChange: function(e) {
            this.setState({
                selectedProfile: e.currentTarget.value
            });
        },

        collectProfileButtonClicked: function() {
            // When collecting recent draw, it will return itself in 1000ms
            if (this.state.collectingProfile && this.state.selectedProfile === 3) return;
        ...
        getProfileMain: function() {
            const buttonText = this.state.collectingProfile ? "Stop" : "Start";
        ...
        render: function() {
            ...
            if (this.state.selectedTab === 0) {
                ...
                const profile = this.state.profiles[this.state.selectedTab - 1];
                if (profile[0] === 4) {
